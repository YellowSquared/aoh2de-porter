 ...
 *
 * Usage: FileLimitPatcher.patch(inputJar, outputJar);
 */
public class FileLimitPatcher {

    private static final boolean RUNTIME_DEBUG = true;
        ...
        try (ZipInputStream zin = new ZipInputStream(new FileInputStream(input));
            ...
            while ((entry = zin.getNextEntry()) != null) {
                ...
                if (entry.getName().endsWith(".class")) {
                    data = patchClass(data, entry.getName());
                }
    ...
    private static byte[] patchClass(byte[] data, String className) {
